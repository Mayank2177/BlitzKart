...
const ProductCard = ({ name, price, originalPrice, unit, emoji, discount }: ProductCardProps) => {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-4 flex flex-col gap-3 hover:shadow-lg transition-shadow relative group">
      {discount && (
        <span className="absolute top-3 left-3 bg-primary text-primary-foreground text-xs font-bold px-2 py-0.5 rounded-md">
          {discount}% OFF
        </span>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        {quantity === 0 ? (
          <Button
            ...
            onClick={() => addItem({ name, price, originalPrice, unit, emoji })}
          >
